defmodule ExAliyunOts.Filter do
  ...
  defp do_serialize_filter(%Filter{type: FilterType.single_column(), filter: filter} = wrapper) do
    ...
    {iodata, _size} = SingleColumnValueFilter.encode!(%{filter | column_value: column_value})
    filter = iodata |> IO.iodata_to_binary()
  ...
  defp do_serialize_filter(%Filter{type: FilterType.composite_column(), filter: filter} = wrapper) do
    ...
    {iodata, _size} = CompositeColumnValueFilter.encode!(%{filter | sub_filters: sub_filters})
    filter = iodata |> IO.iodata_to_binary()
    %{wrapper | filter: filter}
  end
  ...
  defp do_serialize_filter(
    ...
    {iodata, _size} = ColumnPaginationFilter.encode!(filter)
    %{wrapper | filter: IO.iodata_to_binary(iodata)}
  end
